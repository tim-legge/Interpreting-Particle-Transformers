...
from torch.nn import Parameter, Tensor
...
class Gumbel_MultiheadAttention(nn.Module):
...
def multi_head_attention_forward(
    ...
    if need_weights:
        ...
        # Use memory-efficient gumbel_softmax instead of standard one
        attn_output_weights = gumbel_softmax(
            attn_output_weights, tau=0.1, dim=-1
        )
        if dropout_p > 0.0:
...
class Block(nn.Module):
    def __init__(self, embed_dim=128, num_heads=8, ffn_ratio=4,
        ...
        self.attn = Gumbel_MultiheadAttention(
            embed_dim,
            num_heads,
