# This script carries out the batched loading of the JetClass Kinematic dataset and stores it in a specified 
...
if os.path.getsize('/path/to/storage/vol_jc_kin_data/jc_kin_pf_points.npy') == 0:
    ...
    for type, array in jc_kin_data.items():
        with open(f'/path/to/storage/vol_jc_kin_data/jc_kin_{type}.npy', 'wb') as f:
            np.save(f, array)
    ...
    jc_kin_data = {
        'pf_points': np.load('/path/to/storage/vol_jc_kin_data/jc_kin_pf_points.npy'),
        'pf_features': np.load('/path/to/storage/vol_jc_kin_data/jc_kin_pf_features.npy'),
        'pf_vectors': np.load('/path/to/storage/vol_jc_kin_data/jc_kin_pf_vectors.npy'),
        'pf_mask': np.load('/path/to/storage/vol_jc_kin_data/jc_kin_pf_mask.npy'),
        'labels': np.load('/path/to/storage/vol_jc_kin_data/jc_kin_labels.npy')}
...
if not os.path.exists('/path/to/storage/jc_kin_counter.txt'):
    counter = 0
    with open('/path/to/storage/jc_kin_counter.txt', 'w') as f:
        f.write(str(counter))
else:
    print('Counter file exists, resuming from last batch')
    with open('/path/to/storage/jc_kin_counter.txt', 'r') as f:
    ...
    while counter < 51:
        ...
        for jet in range(len(jc_kin_attention[0][:])):
            if np.argmax(jc_kin_labels[jet][:]) == 8: # Hadronic Top
                ...
                continue
        np.save(f'/path/to/storage/batched_attns/jc_kin_topsonly_attention_batch_{counter}.npy', jc_kin_topsonly_attention)
        print(f'Saved {len(jc_kin_topsonly_attention)} top jets attention for batch {counter}')
        np.save(f'/path/to/storage/batched_attns/jc_kin_qcdonly_attention_batch_{counter}.npy', jc_kin_qcdonly_attention)
...
if not os.path.exists('/path/to/storage/jc_kin_topsonly_dist_counter.txt'):
    jc_kin_topsonly_dist_counter = 0
    with open('/path/to/storage/jc_kin_topsonly_dist_counter.txt', 'w') as f:
        f.write(str(jc_kin_topsonly_dist_counter))
else:
    print('Distribution counter file exists, resuming from last batch')
    with open('/path/to/storage/jc_kin_topsonly_dist_counter.txt', 'r') as f:
...
if jc_kin_topsonly_dist_counter >= 51:
    ...
    while jc_kin_topsonly_dist_counter < 51:
        ...
        attention = np.load(f'/path/to/storage/batched_attns/jc_kin_topsonly_attention_batch_{jc_kin_topsonly_dist_counter}.npy', allow_pickle=True)
        ...
        np.save(f'/path/to/storage/batched_hists/jc_kin_topsonly_hist_distribution_batch_{jc_kin_topsonly_dist_counter}.npy', jc_kin_probabilities)
...
if not os.path.exists('/path/to/storage/jc_kin_qcdonly_dist_counter.txt'):
    jc_kin_qcdonly_dist_counter = 0
    with open('/path/to/storage/jc_kin_qcdonly_dist_counter.txt', 'w') as f:
        f.write(str(jc_kin_qcdonly_dist_counter))
else:
    print('Distribution counter file exists, resuming from last batch')
    with open('/path/to/storage/jc_kin_qcdonly_dist_counter.txt', 'r') as f:
...
if jc_kin_qcdonly_dist_counter >= 51:
    ...
    while jc_kin_qcdonly_dist_counter < 51:
        ...
        attention = np.load(f'/path/to/storage/batched_attns/jc_kin_qcdonly_attention_batch_{jc_kin_qcdonly_dist_counter}.npy', allow_pickle=True)
        # Flatten the list of arrays into a single array
        # first check if attention is empty
        if len(attention) == 0:
        ...
        # Process the data in chunks and compute histogram
        jc_kin_probabilities = process_in_chunks(attention_iter, chunk_size=100000, bin_edges=bin_edges)
        np.save(f'/path/to/storage/batched_hists/jc_kin_qcdonly_hist_distribution_batch_{jc_kin_qcdonly_dist_counter}.npy', jc_kin_probabilities)
