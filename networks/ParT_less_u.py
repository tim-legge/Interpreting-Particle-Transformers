...
class ParticleTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.u_blocks = nn.ModuleList([Block(**cfg_block) for _ in range(0, num_u_layers if num_u_layers <= num_layers else num_layers)])
        self.non_u_blocks = nn.ModuleList([Block(**cfg_block) for _ in range(num_u_layers, num_layers)]) if num_u_layers < num_layers else None
    ...
    def forward(self, x, v=None, mask=None, uu=None, uu_idx=None):
        ...
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            if self.non_u_blocks is not None:
                for block in self.non_u_blocks:
