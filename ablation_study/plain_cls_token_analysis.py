...
model_path = '../models/ParTNoPairs_full.pt'
model = mu.get_plain_model('jc_full')
state_dict = torch.load(model_path, map_location='cpu')
model.load_state_dict(state_dict)

cls_hook = mu.ParT_Hook(model, type='class token')
...
model.eval()
with torch.no_grad():
    y_pred = model(torch.from_numpy(jc_full_pf_points), torch.from_numpy(jc_full_pf_features), 
...
#plt.show()
plt.savefig(f'./pca_plots/pca_cls_tokens_plain_model.png')
subprocess.run(['sudo', 'cp', f'./pca_plots/pca_cls_tokens_plain_model.png', storage_path])